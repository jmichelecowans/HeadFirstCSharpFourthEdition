...
namespace JimmyLinqUnitTests
{
    ...
    public class ComicAnalyzerTests
    {
        ...
        public void ComicAnalyser_Should_Generate_A_List_Of_Reviews()
        {
            ...
            {
                $"MuddyCritic rated #1 'Issue 1' {14.5.ToString("0.00", provider)}",
                $"RottenTornadoes rated #1 'Issue 1' {59.93.ToString("0.00", provider)}",
                $"MuddyCritic rated #2 'Issue 2' {40.3.ToString("0.00", provider)}",
                $"RottenTornadoes rated #2 'Issue 2' {95.11.ToString("0.00", provider)}",
            };
        ...
        public void ComicAnalyzer_Should_Handle_Weird_Review_Scores()
        {
            ...
            var expectedResults = new[]
            {
                $"MuddyCritic rated #1 'Issue 1' {(-12.1212).ToString("0.00", provider)}",
                $"RottenTornadoes rated #1 'Issue 1' {391691234.48931.ToString("0.00", provider)}",
                $"RottenTornadoes rated #2 'Issue 2' {0.ToString("0.00", provider)}",
                $"MuddyCritic rated #2 'Issue 2' {40.3.ToString("0.00", provider)}",
                $"MuddyCritic rated #2 'Issue 2' {40.3.ToString("0.00", provider)}",
                $"MuddyCritic rated #2 'Issue 2' {40.3.ToString("0.00", provider)}",
                $"MuddyCritic rated #2 'Issue 2' {40.3.ToString("0.00", provider)}",
