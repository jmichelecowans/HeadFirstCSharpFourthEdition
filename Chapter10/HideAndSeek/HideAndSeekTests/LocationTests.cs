...
namespace HideAndSeekTests
{
    ...
    public class LocationTests
    {
        ...
        public void Initialize()
        {
            ...
            center.AddExit(Direction.In, new Location("Inner Room"));
        ...
        public void TestExitList()
        {
            ...
            var exitList = Enumerable.Empty<string>()
                .Append("the North Room is to the North")
                .Append("the South Room is to the South")
                .Append("the East Room is to the East")
                .Append("the West Room is to the West")
                .Append("the Northeast Room is to the Northeast")
                .Append("the Southwest Room is to the Southwest")
                .Append("the Southeast Room is to the Southeast")
                .Append("the Northwest Room is to the Northwest")
                .Append("the Upper Room is Up")
                .Append("the Lower Room is Down")
                .Append("the Inner Room is In")
                .Append("the Outside Room is Out");

            CollectionAssert.AreEqual(center.ExitList.ToList(), exitList.ToList());
        }
        ...
        public void TestReturnExits()
        {
            ...
            foreach (var exit in center.Exits)
            {
                Assert.AreEqual(center, exit.Value.Exits[(Direction)(-(int)exit.Key)]);
                Assert.AreEqual(exit.Key,
                    exit.Value.GetExit((Direction)(-(int)exit.Key)).Exits.Where(p => p.Value == exit.Value).FirstOrDefault().Key);
            }
        ...
        public void TestAddHall()
        {
            ...
            Assert.AreEqual(eastHall1, eastHall1.GetExit(Direction.Northeast));
        }
    }
}
